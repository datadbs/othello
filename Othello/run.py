...
class ReplayBuffer:
    ...
    def sample(self):
        sample = random.sample(self.buffer, argsDQN.batch_size)
        states, actions, rewards, next_states, done = map(np.asarray, zip(*sample))
        states = np.array(states).reshape(argsDQN.batch_size, -1)
        next_states = np.array(next_states).reshape(argsDQN.batch_size, -1)
...
class ActionStateModel:
    ...
    def create_model(self):
        ...
        model.compile(loss='mse', optimizer=Adam(argsDQN.lr))
        return model
...
class Agent:
    ...
    def replay(self):
        for _ in range(10):
            ...
            targets[range(argsDQN.batch_size), actions] = rewards + (1 - done) * next_q_values * argsDQN.gamma
    ...
    def train(self, ep: object = 0, max_episodes: object = 100) -> object:
        for ep in range(ep,max_episodes):
            ...
            while not done:
                ...
                total_reward += reward
                state = next_state
            if self.buffer.size() >= argsDQN.batch_size:
                self.replay()
            if (ep % argsDQN.target_update_freq) == 0:
                self.target_update()
            print('EP{} EpisodeReward={}'.format(ep, total_reward))
            if ((ep % argsDQN.temp_save_freq) == 0 and argsDQN.temp_save):
               self.db_save(
                path="./save",
...
def create_policy(policy_type='rand', board_size=8, seed=0, search_depth=1,rand_board_count=0):
    if policy_type == 'rand':
        ...
        #policy.load(path="./trans_save8_8", model_name="trans_DQN", version="8.8", num_trained=110000)
...
def play(protagonist,
    ...
    for i in range(num_rounds):
        ...
        while not done:
            action = protagonist_policy.get_action(obs,env.rand_state)
